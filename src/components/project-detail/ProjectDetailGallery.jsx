...
export const ProjectDetailGallery = ({ project }) => {
    ...
    return (
        <section
            ref={galleryRef}
            className="relative px-4 pb-10 pt-20 md:px-8 md:pb-16"
        >
            <div className="mx-auto max-w-375">
                <div className="relative">
                    ...
                    >
                        {/* PREVIEW LABEL */}
                        <div className="absolute left-5 top-5 z-20 flex items-center gap-3 rounded-full border border-white/30 bg-[#f7f5f1]/85 px-4 py-2.5 shadow-sm backdrop-blur-md md:left-7 md:top-7">
                        ...
                        {/* IMAGE */}
                        <div
                            className={`transition-transform duration-1400 ease-out ${imageScale}`}
                        >
                            <ImageCarousel
                                images={images}
                                alt={`${title} preview`}
                        ...
                        {/* IMAGE COUNTER */}
                        {hasMultipleImages && (
                            <div className="absolute bottom-5 right-5 z-20 rounded-full border border-white/10 bg-[#292725]/80 px-4 py-2 font-mono text-[9px] tracking-[0.15em] text-white/80 backdrop-blur-md md:bottom-7 md:right-7">
                    ...
                    {/* GALLERY INFO */}
                    <div
                        className={`mt-5 flex items-center justify-between gap-8 px-1 transition-all delay-300 duration-700 ${labelReveal}`}
                    >
                        <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#918981]">
                            Selección visual del proyecto
                        </p>

                        {hasMultipleImages && (
                            <span className="font-mono text-[9px] uppercase tracking-[0.18em] text-[#918981]">
                                {String(images.length).padStart(2, "0")} vistas
                            </span>
                        )}
                    </div>
                </div>
